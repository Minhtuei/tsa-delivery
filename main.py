...
class CreateDeliveries:
    def __init__(self, new_orders=False, max_weight=10.0):
        ...
        # self.sort_orders_by_TSP_parallel()
    ...
    def sort_orders_by_TSP_parallel(self):
        start_time = time.time()
        tasks = []

        # Gom nhóm đơn hàng cần chạy TSP
        for dormitory, dates in self.grouped_order_by_weight.items():
            for date, time_slots_dict in dates.items():
                for time_slots, trips in time_slots_dict.items():
                    for trip in trips:
                        tasks.append((dormitory, date, time_slots, trip))

        # Dùng ProcessPoolExecutor để chạy song song
        with ProcessPoolExecutor() as executor:
            results = executor.map(self.solve_tsp_for_trip, tasks)

        # Gom kết quả lại vào `self.sorted_orders_by_TSP`
        for dormitory, date, time_slots, sorted_trip in results:
            self.sorted_orders_by_TSP.setdefault(dormitory, {}).setdefault(date, {}).setdefault(time_slots, []).append(sorted_trip)
...
if __name__ == '__main__':
    start_time = time.time()
    CreateDeliveries(new_orders=False)
