...
class TSPSolver:
    ...
    def get_distance(self, order1, order2, alpha=1.0,beta =100.0):
        ...
        if from_building != to_building:
            distance = distance = next((d for d in self.map_box_distance if d["from"] == from_building and d["to"] == to_building), None)
            horizontal_distance = distance["distance"] if distance else 0.0
            vertical_distance = abs(int(order1['room'][0]) + int(order2['room'][0])) * alpha   
        else:
    ...
    def create_distance_matrix(self):
        ...
        for i in range(n):
            for j in range(n):
                if i != j:
                    distance_matrix[i][j] = self.get_distance(self.orders[i], self.orders[j])
